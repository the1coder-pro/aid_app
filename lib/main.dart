...
class _MyHomePageState extends State<MyHomePage> {
    ...
    return Slidable(
      ...
      child: OpenContainer(
        ...
        closedBuilder: (context, _) => Directionality(
          textDirection: TextDirection.rtl,
          child: recordListTile(colorChangeProvider, context, person),
        ),
      ),
    );
  }

  Padding noRecordsPage() {
    return Padding(
  ...
  PopupMenuButton<PopupMenuItemsEnum> moreOptionsButton(BuildContext context,
      TheThemeProvider themeChange, ThemeColorProvider colorChangeProvider) {
    return PopupMenuButton<PopupMenuItemsEnum>(
        ...
        setState(() {
          ...
          switch (selectedMenu!) {
              ...
              Navigator.push(
                  ...
                  PageTransition(
                      child: Directionality(
                          textDirection: TextDirection.rtl, child: ChartPage()),
            ...
            case PopupMenuItemsEnum.settingsPage:
              settingsPage(context, themeChange, colorChangeProvider);
              break;
          }
        });
      },
  ...
  ListTile recordListTile(ThemeColorProvider colorChangeProvider,
      BuildContext context, Person person) {
    return ListTile(
      leading: CircleAvatar(
        backgroundColor: colorChangeProvider.colorTheme == 0
            ? const Color(0xFFe9e9ea)
            : Theme.of(context).colorScheme.primary,
        foregroundColor: colorChangeProvider.colorTheme == 0
        ...
        child: Align(
          ...
          child: Text(
            ...
            style: const TextStyle(fontSize: 20, fontFamily: "ScheherazadeNew"),
          ),
        ),
      ),
      title: Text(
        person.name.split(' ').length > 3
            ...
            : person.name,
        textDirection: TextDirection.rtl,
        style: const TextStyle(fontFamily: "ibmPlexSansArabic", fontSize: 18),
      ),
      subtitle: RichText(
          text: TextSpan(
            ...
            if (person.phoneNumber.isNotEmpty)
              ...
              const TextSpan(),
            TextSpan(
                text: person.aidType != 'عينية' && person.aidType != 'رمضانية'
                                          ...
                                          aidDates: splitedDates.isNotEmpty
                                              ? [
                                                  intl.DateFormat("dd/MM/yyyy")
                                                      .parse(splitedDates[0]),
                                                  intl.DateFormat("dd/MM/yyyy")
                                                      .parse(splitedDates[1]),
                                                ]
                                              : [],
                                          aidType: record.aidType ?? 'غير محدد',
                                          aidAmount: record.aidAmount ?? 0.0,
                                          isContinuousAid:
                                              record.isContinuousAid ?? true,
                  ...
                  OutlinedButton(
                      ...
                      onPressed: () {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                            ...
                            backgroundColor:
                                Theme.of(context).colorScheme.primary,
                            content: Text("تم حذف ${person.name} بنجاح",
                                style: const TextStyle(fontSize: 15))));
                      })
                ],
              ),
